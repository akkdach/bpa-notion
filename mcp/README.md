...
| `append_content` | ต่อท้ายย่อหน้าเข้าไปใน **เนื้อหาของหน้าจริง** (ย่อหน้าธรรมดาเท่านั้น) |
...
**AI อ่านเนื้อหาหน้าได้ และเขียนต่อท้ายได้ (ย่อหน้าธรรมดา)**
...
### ขาเขียน — `append_content` (ย่อหน้าธรรมดาเท่านั้น)

เซิร์ฟเวอร์เขียน Yjs ได้แล้วผ่าน [YDotNet](https://www.nuget.org/packages/YDotNet)
(binding ของ `yrs` ที่เขียนด้วย Rust) แต่**จงใจจำกัดไว้ที่ย่อหน้าธรรมดา** — ไม่มีหัวข้อ
รายการ ตาราง ตัวหนา/เอียง หรือการซ้อนชั้น และต่อท้ายอย่างเดียว แก้/ลบของเดิมไม่ได้

ไม่ใช่ความขี้เกียจ: schema ของ BlockNote นิยามใน TypeScript และเป็น 0.x ที่ minor bump
breaking ได้ การ clone มาเป็น C# ทั้งชุดคือการลอก `blockToNode.ts` + block spec ทุกตัว
ด้วยมือโดยไม่มีอะไรตรวจว่าตรงกัน (`@blocknote/server-util` ตายที่ 0.27.2 ขณะที่โปรเจกต์
pin 0.52.1) ส่วนย่อหน้าธรรมดามีรูปร่างเดียวและตรวจได้ครบด้วยเทสจริง
