...
> [`docs/RB-connect-ai.md`](../docs/RB-connect-ai.md) ซึ่งเป็นคู่มือสำหรับผู้ใช้
>
...
| `replace_content` | **เขียนทับเนื้อหาทั้งหน้า** ด้วย markdown ชุดใหม่ — tool เดียวที่ทำให้ของเดิมหายได้ (`Destructive = true`) ต้องอ่านหน้าก่อนเขียนเสมอ |
...
`add_note` เป็น **append-only** แก้หรือลบไม่ได้ (บันทึกที่แก้ย้อนหลังได้ไม่ใช่บันทึก —
เขียนผิดให้เขียนใหม่ต่อท้าย) ส่วนเนื้อหาหน้าแก้ได้ทาง `replace_content` ซึ่งเขียนทับ
ทั้งหน้า — ประวัติยังอยู่ใน update log แต่ไม่มีเครื่องมือกู้คืนให้ผู้ใช้กดเอง
...
เซิร์ฟเวอร์ (NestJS, `server/`) แปลง markdown ด้วย parser ของ BlockNote ตัวจริง
(`tryParseMarkdownToBlocks`) แล้วเขียนเป็น delta ลง Yjs — schema เดียวกับเบราว์เซอร์โดยนิยาม

| รับได้ | ลดรูปแล้วเตือนกลับ |
|---|---|
| หัวข้อ (`#`–`######`) · ย่อหน้า · รายการ (`-`, `1.`, `- [x]`) ซ้อนได้ 3 ชั้น | HTML ดิบ → บล็อกโค้ด |
| คำพูดอ้างอิง (`>`) · เส้นคั่น (`---`) · บล็อกโค้ด (` ``` `) | ซ้อนลึกเกิน 3 ชั้น → ปรับให้ตื้นลง |
| ตัวหนา · เอียง · ขีดฆ่า · โค้ด · ลิงก์ | |
| **ตาราง GFM** → บล็อก `table` จริง · **รูป** `![ชื่อ](url)` → บล็อก `image` (ทาง markdown ใส่เป็น URL — ผู้ใช้เว็บอัปโหลดไฟล์ตรงได้ผ่าน `POST /files`) | |
| ` ```mermaid ` → **แผนภาพจริงในเบราว์เซอร์** | |

ของที่ลดรูปตอบ **200 พร้อม `warnings`** ไม่ใช่ 400 — ผู้เรียกคือโมเดลที่มองผลลัพธ์ไม่เห็น
การเงียบแปลว่ามันเชื่อว่าเขียนลงไปครบแล้ว ส่วน 400 มักได้ retry ด้วยเนื้อหาเดิม

ข้อความในตารางเข้า index ค้นหาแบบแถวละบรรทัด เซลล์คั่นช่องว่าง — กฎเดียวกันสามที่
(`tableRowLines`/`tableContentLines` ใน `server/src/documents/blocknote.ts` และ
`tableText` ใน `web/.../PageEditor.tsx`) ถ้าแก้ต้องแก้พร้อมกัน ไม่งั้น body_text
จะสลับไปมาทุกครั้งที่มีคนเปิดหน้า
